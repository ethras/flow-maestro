...
2. **Research Log (mandatory)**
   ...
   - Promote reusable, cross-change insights into `.flow-maestro/projects/<project>/constitution.md` once you’ve validated them. Run `flowm projects constitution record "<title>" --summary "<insight>" --source "<change>/<path>:<line>" --section <core|data|operations|risks|watchlist>` so the CLI formats the entry, stamps the verification date, and prevents duplicates. Keep speculative notes in `notes/` until they harden.
